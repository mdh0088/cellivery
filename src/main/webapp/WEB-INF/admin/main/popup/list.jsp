		...
		success : function(data) 
		{
			...
			if(result.list.length > 0)
			{
				...
				for(var i = 0; i < result.list.length; i++)
				{
					...
					inner+='			<span class="onf_btn_text off">OFF</span>';
					inner+='			<span class="onf_btn_text on_2">ON</span>';
					inner+='		</label>';
					inner+='	</td>';
					inner+='	<td><input type="button" value="수정" class="modi_btn btn" onclick="javascript=location.href=\'/admin/main/popup/write?idx='+result.list[i].idx+'\' "></td>';
					inner+='</tr>';
				}
			}
			else
	...
	<div class="inner">
		...
		<div class="inner_cont">
			<div class="list_cont_top">
				...
				<div class="btn_box">
					<input type="button" value="선택항목 삭제" onclick="delTarget()">
					<input type="button" value="선택항목 복사" onclick="duplTarget()">
				</div>
				<input type="button" class="submit_btn btn" value="등록" onclick="javascript:location.href='/admin/main/popup/write'">
			</div>
		
			<div class="list_cont_bottom">
				<table id="lists" class="mainPr_list pr_list">
					<colgroup>
						<col width="5%"/>
						<col width="5%"/>
						<col width=""/>
